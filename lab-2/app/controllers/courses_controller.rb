...
def decision
	...
	if params[:id].to_i > 0  # /courses/(course_id)
		@course = set_course
		show
	elsif (params[:id] == "reload") && checkAdmin  # /courses/reload, requires admin role
		reload
	elsif (params[:id] == "reload") && !checkAdmin # throw error if not an admin...
		flash[:user_error] = 'You do not have proper permissions to access this feature.'
		redirect_to '/error'
	end 
end
...
def update
  ...
  private def parseCourses(courses)
    courses.each do |course|
      #save course info into variables
      course_courseNumber = course["course"]["catalogNumber"]
      course_courseName = course["course"]["title"]
      #The original API splits courses by term, we don't. 
      #This unless statement will combine courses if one already exists, but for a different semester.
      ...
      sections.each do |section|
        #save section info into variables
        ...
        #NOTE: term is saved in the section, not the course. 
        Section.create(sectionNumber: section_sectionNumber, course_id: course_object.id, buildingAndRoom: section_buildingAndRoom, instructor: section_instructor, instructorEmail: section_instructorEmail)
      end
    end
  end
